...
<!-- 编辑出清商品 -->
<div class="pop_editcq" style="display: none">
<form action="" method="post" enctype="multipart/form-data" name="editcq">
	<div class="pop_con" style="width: 550px">
		<p class="pop_tit">
			编辑出清商品
			<span class="pop_close" onclick="$('.pop_editcq').hide()"></span>
		</p>
		<div class="pop_conwrap" style="height: 400px;overflow-y:scroll; background: none">
			<div class="pop_namewrap clearfix">
				<span class="pop_labelname" style="width: 80px">标题</span>
			...
			<div class="pop_namewrap clearfix">
				...
				<div class="vr_line">
					<div class="vr-left"><span>位置</span></div>
					<div class="vr-right r3">
						<span style="font-size: 14px;color:#a1a1a1; cursor: pointer;" id="cqposition"></span>
						<a href="javascript:;" class="pop_buildbtn detail_filebtn detail_filebtn_cpadding" style="margin: 0 0 0 10px" title="设置位置" onclick="$('.r4').show();$('.r3').hide()">设置位置</a>
					</div>
					<div class="vr-right r4" style="display: none">
						<form class="form-inline">
					      <div id="distpicker_cq_edit">
					        <div class="form-group">
					          <label class="sr-only" for="province_cq_edit">Province</label>
					          <select class="form-control form-se-cq_edit"  id="province_cq_edit"></select>
					        </div>
					        <div class="form-group">
					          <label class="sr-only" for="city_cq_edit">City</label>
					          <select class="form-control form-se-cq_edit"  id="city_cq_edit"></select>
					        </div>
					        <div class="form-group">
					          <label class="sr-only" for="district_cq_edit">District</label>
					          <select class="form-control form-se-cq_edit"  id="district_cq_edit"></select>
					        </div>
					        <input type="hidden" name="cityid" value="0"></input>
					      </div>
			    		</form>
						<script type="text/javascript">
							$("#distpicker_cq_edit").distpicker({
							  province: "---- 省 ----",
							  city: "---- 市 ----",
							  district: "---- 区县 ----",
							  autoSelect: false
							...
							$('.form-se-cq_edit').change(function(){
								var s1 = $('.form-se-cq_edit').eq(0).find('option:selected').attr('data-code')
								var s2 = $('.form-se-cq_edit').eq(1).find('option:selected').attr('data-code')
								var s3 = $('.form-se-cq_edit').eq(2).find('option:selected').attr('data-code')
								if(s1!='' && s2==''){
									$('input[name=cityid]',$('#distpicker_cq_edit')).val(s1)
								}
								if(s1!='' && s2!='' && s3==''){
									$('input[name=cityid]',$('#distpicker_cq_edit')).val(s2)
								}
								if(s1!='' && s2!='' && s3!=''){
									$('input[name=cityid]',$('#distpicker_cq_edit')).val(s3)
								}
								if(s1=='' && s2=='' && s3==''){
									$('input[name=cityid]',$('#distpicker_cq_edit')).val("0")
								}
							})
						</script>		
					</div>
				...
				<div class="pop_namewrap clearfix">
					<span class="pop_labelname" style="width: 80px">当前分类</span>
					<span style="font-size: 14px;color:#a1a1a1;" id="ccate"></span>
				</div>
				<div class="pop_namewrap fenlei clearfix">
					<span class="pop_labelname" style="width: 80px">分类</span>
					<select name="f_select"  class="pop_iptname" style="width: 170px">
				...
				<input type="hidden" name="good_id" value=""></input>
				<input type="hidden" name="user_id" value="<?php if(!empty($_COOKIE['user_id'])) echo $_COOKIE['user_id'];?>"/>
