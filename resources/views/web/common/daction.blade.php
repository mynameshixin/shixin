	...
	<div class="pop_con">
		...
		<div class="pop_conwrap">
			<div class="pop_namewrap clearfix">
				...
				<input class="pop_iptname" placeholder="为这个VR场景添加一个名称和描述" name='title' value="" style="width: 300px">
			</div>
			<div class="pop_namewrap clearfix">
			...
			<div class="pop_namewrap clearfix">
				...
				<input class="pop_iptname" placeholder="粘贴这个VR场景的链接地址" name='detail_url' value="" style="width: 300px">
			</div>
...
<script type="text/javascript">
	...
	$('.hourse').css('display','none');
	$('.shop').css('display','none');
	function changeType(obj){
		var _val=$(obj).attr('data-val');
		if(_val=='4'){
			
			$('.hourse').find('input[type=hidden]').val("0")
			$('.hourse').find('label').removeClass("on")
			for (var i = 0; i < $('select.pop_selects').length; i++) {
				$('select.pop_selects').eq(i).find('option').removeAttr('selected')
			}
			...
			$('.shop').find('input[type=hidden]').val("0")
			$('.shop').find('label').removeClass("on")
			for (var i = 0; i < $('select.pop_selects').length; i++) {
				$('select.pop_selects').eq(i).find('option').removeAttr('selected')
			}
			$('.shop').css('display','none');
			$('.hourse').css('display','block');
		}
