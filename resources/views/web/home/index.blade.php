						...
						<div class="index_item_user">
							<div class="index_item_utop clearfix">
								<div class="index_item_uava">
									<a href="{{url('webd/user')}}" target="_blank" title="{{$user_info['nick']}}"><img src="{{!empty($user_info['auth_avatar'])?$user_info['auth_avatar']:$user_info['pic_m']}}" alt="{{$user_info['nick']}}"></a>
								</div>
								<div class="index_item_uname"><a href="javascript:;" target="_blank" title="{{$user_info['nick']}}"><?php echo mb_substr($user_info['nick'], 0,6,'utf-8') ?></a></div>
							</div>
							...
							<?php foreach ($recommend as $key => $re):?>
								...
								<div class="index_item_ubfold clearfix">
									<div class="index_item_ubfava">
										<a href="{{url('webd/folder')}}?fid={{$re['id']}}" target="_blank" title="{{$re['name']}}"><img src="{{$re['img_url']}}" alt="{{$re['name']}}"></a>
									</div>
									<div class="index_item_ubfinfo">
										<p class="index_item_ubfnme"><a href="{{url('webd/folder')}}?fid={{$re['id']}}" target="_blank" title="{{$re['name']}}"><?php echo mb_substr($re['name'], 0,8,'utf-8') ?></a></p>
				...
				<div class="index_item">
					<div class="index_item_wrap">
						<div class="index_item_imgwrap clearfix">
							<a class="index_item_blurwrap" href="{{url('webd/pic/')}}/{{$v['id']}}" target="_blank" title="{{$v['description']}}"></a>
							<img src="{{$v['images'][0]['img_m'] or url('uploads/sundry/blogo.jpg')}}" style="height: {{$v['images'][0]['rh']}}px" onload="resize_xy(this)" alt="{{$v['description']}}">
						...
						<div class="index_item_info">
							...
							<div class="index_item_bottom clearfix">
								<a href="{{url('webd/user')}}?oid={{$v['user']['id']}}" class="index_item_authava" target="_blank" title="{{!empty($v['user']['nick'])?$v['user']['nick']:$v['user']['username']}}">
								...
								<div class="index_item_authinfo">
									<a href="{{url('webd/user')}}?oid={{$v['user']['id']}}" target="_blank" class="index_item_authname" title="{{!empty($v['user']['nick'])?$v['user']['nick']:$v['user']['username']}}">{{!empty($v['user']['nick'])?$v['user']['nick']:$v['user']['username']}}</a>
