...
<body id="ashow" ng-app="edvr">
	...
	<script type="text/javascript">
  var edvr = angular.module('edvr', [], function($interpolateProvider) {
   $interpolateProvider.startSymbol('{%');
   $interpolateProvider.endSymbol('%}');
  });

  edvr.controller('dev',function($scope,$http){
		$http({
		    method: 'get',
		    url: '/api/vr/dev',
	...
	edvr.controller('huxing',function($scope,$http){
		$http({
		    method: 'get',
		    url: '/api/vr/huxing',
		  }).success(function(json, status) {
	...
	edvr.controller('type',function($scope,$http){
		$http({
		    method: 'post',
		    url: '/api/vr/type',
		  }).success(function(json, status) {
	...
	edvr.controller('btype',function($scope,$http){
		$http({
		    method: 'get',
	...
	edvr.controller('sale',function($scope,$http){
		$http({
		    method: 'get',
		    url: '/api/vr/sales',
		  }).success(function(json, status) {
				...
				<div class="vr_line">
					...
					<div class="vr-right">
						<div class="select hourse-type" ng-controller="type">
							<label data-val="{%v.id%}" ng-repeat="v in type" onclick="changeeType(this)"><i></i>{%v.name%}</label>
	...
	<script type="text/javascript">
		...
		function changeeType(obj){
			...
			if(_val=='4'){
				...
				for (var i = 0; i < $('select.pop_selects',$('.pop_editvr')).length; i++) {
					$('select.pop_selects',$('.pop_editvr')).eq(i).find('option').removeAttr('selected')
				}
