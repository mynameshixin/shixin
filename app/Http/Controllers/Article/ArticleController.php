...
class ArticleController extends CmController{
	...
	public function add_eassat(){  //写入数据库前判定
		...
		if($_FILES['file2']['name']){$file2 = Input::file('file2');$filename = uniqid();$hz=$file2 -> getClientOriginalExtension();move_uploaded_file($_FILES["file2"]["tmp_name"],
      '/uploads/ueditor/show/'.$filename.'.'.$hz);$data['eassat_timg']='/uploads/ueditor/show/'.$filename.'.'.$hz;
		}else{$file1 = Input::file('file1');$filename = uniqid();$hz=$file1 -> getClientOriginalExtension();move_uploaded_file($_FILES["file1"]["tmp_name"],
      '/uploads/ueditor/show/'.$filename.'.'.$hz);$data['eassat_timg']='/uploads/ueditor/show/'.$filename.'.'.$hz;}}else{dd("请设置推荐图片 11:8");}
		if($_FILES['file3']['name']||$_FILES['file4']['name']){if($_FILES['file4']['name']){$file4 = Input::file('file4');$filename = uniqid();$hz2=$file4 -> getClientOriginalExtension();move_uploaded_file($_FILES["file4"]["tmp_name"],
      '/uploads/ueditor/show/'.$filename.'.'.$hz);$data['eassat_ximg']='/uploads/ueditor/show/'.$filename.'.'.$hz2;
		}else{$file3 = Input::file('file3');$filename = uniqid();$hz2=$file3 -> getClientOriginalExtension();move_uploaded_file($_FILES["file3"]["tmp_name"],
      '/uploads/ueditor/show/'.$filename.'.'.$hz);$data['eassat_ximg']='/uploads/ueditor/show/'.$filename.'.'.$hz2;}
